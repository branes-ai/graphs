...
| `kpu_power_model.py` | Per-kind energy. **pe_fabric**: datapath ops x EnergyRef, plus the existing per-PE structures (token match, regs, forward, clock), plus overlay traffic. **systolic**: MAC plus buffers. **fixed_function**: units/s x pj_per_unit scaled to the node. Per-domain V/f, gating, `tdp_scenario`. The legacy path is unchanged. |
