...
def _safe_peak_gflops(mapper, precision: Precision) -> float:
    """Return peak GFLOPS for a precision, or 0.0 if the mapper does not list it.

    This survey CLI walks every registered mapper and prints peak performance
    at FP32 / INT8 (etc.). Since #53, ``get_peak_ops`` raises on unsupported
    precisions instead of silently falling back, so this helper translates the
    raise into a 0.0 cell (= "not natively supported") for table rendering.
    Estimation paths that genuinely need the value should NOT use this helper
    -- they should let the ValueError propagate.
    """
    try:
