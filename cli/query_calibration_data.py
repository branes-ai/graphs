...
def load_hardware_data(hardware_id: str) -> Dict[str, Any]:
    """Load all data for a hardware target."""
...
def list_hardware():
    ...
    if not calibration_dir.exists():
        print("  No calibration_data directory found")
        return

    headers = ["Hardware ID", "Models", "Op Types", "Data Points", "Date"]
    print(f"\n  {headers[0]:<25} {headers[1]:>7} {headers[2]:>9} {headers[3]:>12} {headers[4]:<12}")
    print("  " + "-" * 70)
    ...
    for hw_dir in sorted(calibration_dir.iterdir()):
        ...
        print(f"  {hw_id:<25} {model_count:>7} {op_types:>9} {data_points:>12} {date:<12}")

    print()


def show_curves(hardware_id: str):
    """Show efficiency curves for a hardware target."""
    data = load_hardware_data(hardware_id)
    if not data:
        print(f"No data found for {hardware_id}")
        return

    curves = data.get("efficiency_curves")
    if not curves:
        print(f"No efficiency curves found for {hardware_id}")
        return
    ...
    print(f"  Precision: {curves.get('precision', 'unknown')}")
...
def show_model(hardware_id: str, model_name: str):
    """Show measurements for a specific model."""
    data = load_hardware_data(hardware_id)
    if not data:
        print(f"No data found for {hardware_id}")
    ...
    if not measurement:
        print(f"No measurement found for {model_name} on {hardware_id}")
        return
    ...
    print(f"  Precision: {measurement.get('precision', 'unknown')}")
...
def compare_hardware():
    ...
    for op_type in sorted(all_op_types):
        print(f"\n  {op_type}:")
        print(f"    {'Hardware':<25} {'Peak Eff':>10} {'@ FLOPs':>12} {'Data Pts':>10}")
        print(f"    {'-' * 60}")

        for hw_id, curves in sorted(hw_data.items()):
            ...
            print(f"    {hw_id:<25} {peak_eff:>10.3f} {format_flops(peak_flops):>12} {data_pts:>10}")
