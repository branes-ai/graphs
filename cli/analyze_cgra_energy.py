#!/usr/bin/env python
"""
CGRA Energy Breakdown Analysis Tool

Analyzes energy consumption of DNN models mapped to CGRA (Coarse-Grained Reconfigurable Array).
Shows detailed breakdown of CGRA-specific energy events (PCUs, PMUs, reconfiguration, etc.).

Usage:
    # Basic usage
    ./cli/analyze_cgra_energy.py --model resnet18

    # With custom batch size and precision
    ./cli/analyze_cgra_energy.py --model mobilenet_v2 --batch-size 8 --precision int8

    # JSON output
    ./cli/analyze_cgra_energy.py --model resnet50 --output cgra_energy.json

    # List available models
    ./cli/analyze_cgra_energy.py --list-models
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='CGRA Energy Breakdown Analysis for DNN Models',
    ...
    parser.add_argument(
        ...
        default='int8',
        help='Numerical precision (default: int8 for CGRA)'
    )
...
def main():
    ...
    print("=" * 80)
    print("CGRA ENERGY BREAKDOWN ANALYSIS")
    print("=" * 80)
    print(f"CGRA: Stanford Plasticine-v2 (32 PCUs, 6.14 TOPS INT8, 15W)")
    ...
    # Step 2: Create CGRA mapper
    print(f"\n[2/4] Creating CGRA mapper...")
    try:
        cgra_mapper = create_plasticine_v2_mapper()

        print(f"✓ CGRA mapper created: {cgra_mapper.resource_model.name}")
        print(f"  PCUs: {cgra_mapper.resource_model.compute_units}")
        print(f"  Peak Performance (INT8): {cgra_mapper.resource_model.precision_profiles[Precision.INT8].peak_ops_per_sec / 1e12:.2f} TOPS")
    except Exception as e:
        print(f"✗ Failed to create CGRA mapper: {e}")
        sys.exit(1)

    # Step 3: Map model to CGRA
    print(f"\n[3/4] Mapping model to CGRA...")
    try:
        ...
        # Map to CGRA hardware
        mapping_result = cgra_mapper.map_graph(
            partition_report,
            execution_stages,
            batch_size=args.batch_size,
            precision=precision
        ...
        print(f"  PCU utilization: {mapping_result.average_utilization * 100:.1f}%")
    ...
    arch_events = None
    if cgra_mapper.resource_model.architecture_energy_model:
        # Aggregate ops and bytes across all subgraphs
        total_ops = sum(alloc.compute_time * cgra_mapper.resource_model.get_peak_ops(precision)
        ...
        execution_context = {
            'batch_size': args.batch_size,
            'num_pcus': cgra_mapper.resource_model.compute_units,
        }

        try:
            arch_breakdown = cgra_mapper.resource_model.architecture_energy_model.compute_architectural_energy(
                ops=int(total_ops),
    ...
    if arch_events:
        from energy_breakdown_utils import print_cgra_hierarchical_breakdown

        total_compute_energy = sum(a.compute_energy for a in mapping_result.subgraph_allocations)
        total_memory_energy = sum(a.memory_energy for a in mapping_result.subgraph_allocations)

        print_cgra_hierarchical_breakdown(
        ...
        if not cgra_mapper.resource_model.architecture_energy_model:
            print(f"\nNote: Detailed architectural breakdown not available")
            print(f"      (no CGRAEnergyModel configured)")
    ...
    if args.output:
        ...
        export_energy_results(
            ...
            architecture='CGRA',
            hardware_name=cgra_mapper.resource_model.name,
