...
private:
     ...
     *
     * Loop nest: i-j-k where each i-j block processes k tiles sequentially
     * Each k iteration updates the same C[i,j] tile (accumulated in memory)
     */
    static void multiply_l2_tile(const Matrix& A, const Matrix& B, Matrix& C,
        ...
        for (size_t i = i0; i < i_end; i += L1_TILE) {
            ...
            for (size_t j = j0; j < j_end; j += L1_TILE) {
                ...
                // For this i×j tile, process all K in L1-sized chunks
                for (size_t k = k0; k < k_end; k += L1_TILE) {
                    size_t k1_end = std::min(k + L1_TILE, k_end);

                    // Micro-kernel processes this i×j×k block
                    // Accumulates in registers across k dimension
    ...
    /**
     * @brief L1 cache blocking - calls micro-kernel with register accumulation
     *
     * For each i×j×k block, accumulate C in SIMD registers across the k dimension
     */
    static void multiply_l1_tile(const Matrix& A, const Matrix& B, Matrix& C,
                                 ...
                                 size_t j0, size_t j_end,
                                 size_t k0, size_t k_end) {
        // Process each row in the i-tile
        for (size_t i = i0; i < i_end; ++i) {
            // Process columns in SIMD-width chunks
            size_t j = j0;

            // Main loop: 16 columns (2× AVX2 registers)
            for (; j + 16 <= j_end; j += 16) {
                micro_kernel_16(A, B, C, M, N, K, i, j, k0, k_end);
            }

            // 8-column block
            if (j + 8 <= j_end) {
                micro_kernel_8(A, B, C, M, N, K, i, j, k0, k_end);
                j += 8;
            }
    ...
    /**
     * @brief 16-wide micro-kernel with register accumulation
     *
     * Processes C[i, j:j+16] by accumulating across k0:k_end in TWO AVX2 registers
     * Load/store C ONCE per K-tile, accumulate K iterations in registers
     */
    static void micro_kernel_16(const Matrix& A, const Matrix& B, Matrix& C,
                                size_t M, size_t N, size_t K,
                                size_t i, size_t j,
                                size_t k0, size_t k_end) {
        // Load C[i, j:j+16] if this is a subsequent K-tile
        __m256 c0, c1;
        if (k0 == 0) {
            c0 = _mm256_setzero_ps();
            c1 = _mm256_setzero_ps();
        } else {
            c0 = _mm256_loadu_ps(&C[i * N + j]);
            c1 = _mm256_loadu_ps(&C[i * N + j + 8]);
        }

        // Accumulate across K dimension in registers
        for (size_t k = k0; k < k_end; ++k) {
    ...
    /**
     * @brief 8-wide micro-kernel with register accumulation
     */
    static void micro_kernel_8(const Matrix& A, const Matrix& B, Matrix& C,
                               size_t M, size_t N, size_t K,
                               size_t i, size_t j,
                               size_t k0, size_t k_end) {
        // Load or zero C[i, j:j+8]
        __m256 c0 = (k0 == 0) ? _mm256_setzero_ps() : _mm256_loadu_ps(&C[i * N + j]);
